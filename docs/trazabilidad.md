# Trazabilidad de requerimientos — FR-001..FR-050 y SC-001..SC-026

Documento de trazabilidad (T052) que mapea cada requerimiento funcional y cada
criterio de éxito a su implementación y tests. Verifica que no queda ningún
requerimiento sin su correspondiente tarea/implementación y que cada SC está
verificado por al menos un test.

## Requerimientos funcionales (FR-001..FR-050)
...
| FR-037 | Persistir la conversación (guardar/cargar/listar sesión) | `agent/session_manager.py` (`SesionManager`) | `test_reasoning.py::test_sesion_manager_guardar_y_cargar_json`, `test_reasoning.py::test_sesion_manager_listar_y_borrar` |
| FR-038 | Gestionar tareas (CRUD: crear, listar, estado, prioridad, etiquetas) | `agent/gestor_tareas.py` (`GestorTareas`) | `test_reasoning.py::test_gestor_tareas_crud`, `test_reasoning.py::test_gestor_tareas_filtros_y_dependencias`, `test_reasoning.py::test_gestor_tareas_serializacion` |
| FR-038+ | Ejecutar tareas (`/tarea run`): delegar a ReAct, marcar completada/bloqueada y guardar resultado | `agent/conversational.py` (`ejecutar_tarea`) + `cli/main.py` (`/tarea run`) | `test_reasoning.py::test_ejecutar_tarea_completada_con_resultado`, `test_reasoning.py::test_ejecutar_tarea_bloqueada_sin_evidencia`, `test_reasoning.py::test_ejecutar_tarea_inexistente_o_completada`, `test_reasoning.py::test_ejecutar_tarea_persiste_resultado_al_guardar`, `test_reasoning.py::test_cli_chat_tarea_run_ejecuta_y_guarda_resultado` |
| FR-039 | Mantener historial/memoria entre turnos (contexto conversacional) | `agent/conversational.py` (`_contexto_conversacional` inyecta historial/resumen/tareas en `Intencion.contexto`) + `agent/loop.py` (`atender(..., contexto)`) | `test_reasoning.py::test_agent_conversacional_inyecta_contexto_historial` |
| FR-040 | Delegar análisis QA al bucle ReAct y conversación general directa (no repetitiva, T113) | `agent/conversational.py` (`_es_intencion_qa` + `_responder_directo` inyecta historial en la intención) + prompt de `openai_compatible_backend.responder` (rama sin observaciones) | `test_reasoning.py::test_agent_conversacional_delega_qa_a_react`, `test_reasoning.py::test_agent_conversacional_responde_directo_sin_qa`, `test_reasoning.py::test_agent_conversacional_directo_recibe_pregunta_y_respuesta_previa` |
| FR-041 | Interfaz de chat interactivo con comandos (`/tarea`, `/sesion`, `/memoria`, `/ayuda`) | `cli/main.py` (`chat`, `_ejecutar_comando`, `_comando_tarea`, `_comando_sesion`) | `test_reasoning.py::test_cli_chat_crea_lista_y_actualiza_tareas`, `test_reasoning.py::test_cli_chat_guardar_listar_y_cargar_sesion`, `test_reasoning.py::test_cli_chat_memoria_y_ayuda` |
...
| FR-050 | Ocultar el historial visible por defecto en el CLI, mostrándolo con `--mostrar-historial` (trazabilidad por paso en Razonamiento) | `cli/main.py` (`_renderizar_respuesta(mostrar_historial=False)`, opción `--mostrar-historial` en `main`/`chat`) | `test_cli_render.py::test_render_oculta_el_historial_por_defecto`, `test_cli_render.py::test_render_muestra_el_historial_con_flag` |

## Criterios de éxito (SC-001..SC-026)
...
| SC-018 | Sesión guardada/cargada conserva turnos, tareas y hechos | `test_reasoning.py::test_agent_conversacional_tareas_y_persistencia`, `test_reasoning.py::test_sesion_manager_guardar_y_cargar_json` |
| SC-019 | Chat multi-turno con FakeLLM responde sin romper (QA delegado + directo) | `test_reasoning.py::test_agent_conversacional_delega_qa_a_react`, `test_reasoning.py::test_agent_conversacional_responde_directo_sin_qa`, `test_reasoning.py::test_cli_chat_memoria_y_ayuda` |
| SC-020 | Tarea ejecutada solo con evidencia real: completada con resultado o bloqueada | `test_reasoning.py::test_ejecutar_tarea_completada_con_resultado`, `test_reasoning.py::test_ejecutar_tarea_bloqueada_sin_evidencia` |
...
Los FR-001..FR-050 están mapeados a su implementación y cubiertos por tests;
los SC-001..SC-026 están verificados por al menos un test. La **Phase 14
...
en el bucle ReAct y el chat. La **Phase 15 (profundidad de análisis)** está
...
explora BLL/DAL/EDL/UIL/WebPortal y responde con evidencia real. No queda ningún
requerimiento funcional ni no funcional sin su correspondiente
task/implementación (constitución XIV).